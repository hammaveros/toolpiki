...
interface ResultShareButtonsEnProps {
  url: string;
  title?: string;
  description?: string;
  className?: string;
...
/**
 * Result share buttons component (EN)
 * Only visible when result exists
 * Shows error message if canShare is false
 */
export function ResultShareButtonsEn({
  url,
  title = '',
  description = '',
  className,
  ...
  canShare = true,
  shareError = null,
}: ResultShareButtonsEnProps) {
  const [copied, setCopied] = useState(false);

  if (!visible) return null;

  // Show error message if sharing not possible
  if (!canShare) {
    return (
      <div className={cn('flex items-center gap-2', className)}>
        <span className="text-xs text-gray-400 dark:text-gray-500">
          {shareError === 'TOO_LARGE' ? 'Result too large to share' : 'Failed to generate share URL'}
        </span>
      </div>
    );
  }
  ...
  const handleReddit = () => {
    window.open(`https://reddit.com/submit?url=${encodedUrl}&title=${encodedTitle}`, '_blank', 'width=600,height=400');
  };

  return (
    <div className={cn('flex items-center gap-2', className)}>
      <span className="text-xs text-gray-500 dark:text-gray-400">Share result</span>
