    ...
    <div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 space-y-8 text-gray-700 dark:text-gray-300">
      ...
      <section>
        ...
        <p className="text-sm leading-relaxed mb-3">
          이 룰렛은 단순한 균등 확률이 아닌 '가중 무작위(weighted random)' 방식으로 동작합니다.
          각 항목의 당첨 확률은 <span className="font-mono bg-gray-100 dark:bg-gray-800 px-1 rounded">P(i) = w(i) / Σw</span> 공식으로 계산됩니다.
          예를 들어 A=5, B=3, C=2 가중치를 주면 총합 10이 되고, A는 50%·B는 30%·C는 20% 확률을 갖습니다.
          파이 차트의 조각 크기가 곧 확률이므로 시각적으로도 직관적이며, 룰렛이 멈춘 화살표 위치를 누적 확률 구간에 매핑해 당첨 항목을 결정합니다.
        </p>
        <p className="text-sm leading-relaxed text-gray-600 dark:text-gray-400">
          가중치 1:1:1로 3개 항목을 두면 각 33.3%이고, 1:1:8로 두면 마지막 항목이 80% 확률로 뽑힙니다.
          1~1000 범위 덕분에 0.1%(1:999) 같은 희박 확률부터 균등까지 자유롭게 조절할 수 있습니다.
        </p>
      </section>
      ...
      <section>
        ...
        <ul className="text-sm space-y-2 list-disc list-inside text-gray-600 dark:text-gray-400">
          <li><strong>회식 자리 선정</strong>: 한식·중식·일식·양식·분식을 1:1:1:1:1로 두면 균등, 분식만 5로 두면 분식이 더 자주.</li>
          <li><strong>독서 모임 다음 책</strong>: 추천 책 10권 입력 후 균등으로 돌리면 공정한 추첨.</li>
          <li><strong>이벤트 경품 추첨</strong>: 1등 1, 2등 5, 참가상 30 가중치로 등급별 확률 조절.</li>
          <li><strong>업무 분담</strong>: 모두가 싫어하는 업무에 가중치를 낮게(1), 선호 업무에 높게(10) 두어 부담 분산.</li>
          <li><strong>모바일 친화</strong>: 화면이 작으면 항목 텍스트가 8자에서 자동 잘림 처리, 결과 카드에서 전체 이름 확인 가능.</li>
          <li><strong>결과 공유</strong>: 룰렛이 멈춘 정확한 각도까지 URL에 저장돼, 친구가 링크를 열면 같은 결과로 재현됩니다.</li>
        </ul>
      </section>
        ...
        faqs={[
          {
            ...
            answer: '가중치는 해당 항목의 상대적 당첨 확률을 결정합니다. 예를 들어 A(가중치 2)와 B(가중치 1)가 있으면 A는 약 67%, B는 약 33% 확률로 당첨됩니다. 가중치 합이 분모가 되고 각 항목 가중치가 분자가 되는 단순한 비율 계산입니다.',
          },
          {
            question: '균등한 확률로 하려면?',
            answer: '모든 항목의 가중치를 동일하게(예: 전부 1) 설정하면 N개 항목이 각각 1/N 확률로 동일하게 선택됩니다. 기본값이 1이라 그대로 두면 자동으로 균등 추첨이 됩니다.',
          },
          {
            question: '가중치 범위가 왜 1~1000인가요?',
            answer: '세밀한 확률 조절을 위해 넓은 범위를 제공합니다. 예를 들어 0.1% 확률을 표현하려면 1:999 비율로 설정할 수 있고, 0이나 음수는 의미가 없어 1을 최소값으로 두었습니다.',
          },
          {
            question: '한 번 돌린 결과를 친구에게 그대로 보여주려면?',
            answer: '결과 공유 버튼으로 생성된 링크에는 항목·가중치·당첨 인덱스·최종 각도까지 모두 인코딩되어 있어, 친구가 링크를 열면 같은 룰렛이 같은 자리에서 멈추는 애니메이션을 그대로 볼 수 있습니다.',
          },
        ]}
      />
    </div>
