    ...
    <div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 space-y-8 text-gray-700 dark:text-gray-300">
      ...
      <section>
        ...
        <ul className="text-sm leading-relaxed space-y-2 list-disc list-inside">
          <li><strong>대표색 선택</strong>: 이미지를 5등분해서 각 영역에서 가장 넓은 면적의 색을 한 번씩 뽑으면, 자연스러운 5색 팔레트가 만들어집니다.</li>
          <li><strong>그림자/하이라이트 짝짓기</strong>: 같은 물체의 빛 받는 면(밝은 톤)과 그늘진 면(어두운 톤)을 함께 뽑으면 UI 버튼의 기본/액티브 상태로 그대로 매핑할 수 있습니다.</li>
          <li><strong>고해상도 이미지</strong>: 가로 1200px 이상의 이미지가 좋습니다. 썸네일급 이미지는 안티앨리어싱으로 인해 경계선 픽셀이 의도와 다른 색으로 나옵니다.</li>
          <li><strong>JPEG 압축 주의</strong>: JPEG는 8x8 블록 단위로 색이 평균화되어 단색 영역이라도 가장자리에서 색이 살짝 흔들립니다. 정확도가 중요하면 PNG/WebP(loseless) 권장.</li>
          <li><strong>중앙 픽셀만 보지 말기</strong>: 사진 인물의 피부톤을 뽑을 때 정확히 한 픽셀만 보면 노이즈에 영향을 받습니다. 같은 영역을 3~4번 다른 위치에서 뽑고 평균을 내세요.</li>
          <li><strong>웹/인쇄 색공간</strong>: 이 도구는 sRGB 기준입니다. 인쇄용 CMYK는 별도 변환이 필요하며, 채도 높은 형광색은 그대로 인쇄되지 않을 수 있습니다.</li>
        </ul>
      </section>
        ...
        faqs={[
          {
            ...
            answer: '모니터의 색상 프로필(sRGB/P3/Adobe RGB), 이미지 압축, 색공간 차이로 인해 발생할 수 있습니다. 캘리브레이션된 모니터와 무손실 이미지(PNG, 무압축 WebP)를 사용하면 더 정확한 결과를 얻을 수 있습니다.',
          },
          {
            question: '이미지가 서버로 업로드되나요?',
            answer: '아니요, 모든 처리는 브라우저에서 로컬로 이루어집니다. 이미지는 서버로 전송되지 않으며, 페이지를 닫으면 데이터가 삭제됩니다. 저작권 있는 레퍼런스도 안심하고 분석할 수 있습니다.',
          },
          {
            question: '최대 10개 이상 추출할 수 있나요?',
            answer: '새로운 색상을 추출하면 가장 오래된 색상이 자동으로 삭제됩니다. 필요한 색상은 복사 버튼으로 미리 메모장이나 디자인 툴에 저장해두는 워크플로우를 권장합니다.',
          },
          {
            question: '한 픽셀의 평균색이 아니라 영역 평균을 뽑을 수 있나요?',
            answer: '현재는 클릭한 정확한 픽셀의 색을 가져옵니다. 영역 평균이 필요하면 같은 부근을 여러 번 클릭한 뒤 결과들의 RGB 평균을 직접 내거나, 사전에 이미지를 약간 블러 처리한 뒤 뽑는 방법이 효과적입니다.',
          },
        ]}
      />
    </div>
